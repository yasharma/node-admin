            ...
            link: function ($scope, $element, $attrs, $ngModelCtrl) {
                ...
                var updateSelectionLists = function () {
                    if (!$ngModelCtrl.$viewValue) {
                        ...
                        // suggested by mahendra singh @ 14 july
                        $scope.$watch('resolvedOptions', function (data) {
                            if(data){
                ...
                $scope.getButtonText = function () {
                    ...
                    if ($scope.selectedOptions && $scope.selectedOptions.length > 1) {
                        ...
                        if (totalSelected === 0) {
                            ...
                            /**
                             * Only show if show-item attribute set to true and
                             * selected items length is set to less then 10
                             * @param  {Boolean} $scope.showItem && $scope.selectedOptions.length < 10 [Selected items length]
                             * @return {String}                 [List of names of all the selected items]
                             */
                            if($scope.showItem && $scope.selectedOptions.length < 10 ){
                                return $scope.displayItems($scope.selectedOptions);    
                            }
                ...
                $scope.displayItems = function (object) {
                    if( angular.isObject(object) )  {
                        return object.map(function (item) { return item[$scope.displayProp] }).join(', ');
                    }
                    return object;
                };
